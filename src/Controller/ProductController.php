...
final class ProductController extends AbstractController{
    ...
    {
        return new JsonResponse($this->serializer->serialize($this->em->getRepository(Product::class)->findAll(), 'json', SerializationContext::create()->setGroups($this->isGranted('ROLE_SUPER_ADMIN') ? ['read:product', 'read:product:superadmin'] : ['read:product'])), Response::HTTP_OK, [], true);
    }

    #[Route('/{id}', name: 'product', requirements: ['id' => '\d+'], methods: ['GET'])]
    public function getProduct(Product $product): JsonResponse
    {
        return new JsonResponse($this->serializer->serialize($product, 'json', SerializationContext::create()->setGroups($this->isGranted('ROLE_SUPER_ADMIN') ? ['read:product', 'read:product:superadmin'] : ['read:product'])), Response::HTTP_OK, [], true);
    }
    ...
    public function createProduct(Request $request): JsonResponse
    {
        ...
        return new JsonResponse($this->serializer->serialize($product, 'json', SerializationContext::create()->setGroups($this->isGranted('ROLE_SUPER_ADMIN') ? ['read:product', 'read:product:superadmin'] : ['read:product'])), Response::HTTP_CREATED, ['Location' => $this->urlGenerator->generate('product', ['id' => $product->getId()], UrlGeneratorInterface::ABSOLUTE_URL)], true);
    }
